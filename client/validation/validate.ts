...
export const accountSchema = defineFormSchema({
  ...
  byFather: Yup.string(),
  phone: Yup.string().required(requiredText).min(14, phoneInputError),
...
export const createDeviceSchema = defineFormSchema({
  ...
  discount: Yup.number(),
  shortDescr: Yup.string().required(requiredText),
  image: Yup.string().required(requiredText),
  brandId: Yup.number().required(requiredText),
  typeId: Yup.number().required(requiredText),
  name: Yup.string().required(requiredText),
  longDescr: Yup.string().required(requiredText)
});
...
export const namePhoneSchema = defineFormSchema({
  phone: Yup.string().required(requiredText).min(14, phoneInputError),
});
