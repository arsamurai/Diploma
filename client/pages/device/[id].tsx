...
interface DeviceProps {
  serverDevice: IDevice;
}

const Device: React.FC<DeviceProps> = ({ serverDevice }) => {
  ...
  const [device, setDevice] = useState<IDevice>(serverDevice);
  ...
  return (
    <MainLayout device>
      <div className="device">
        ...
        <h4 className="device__title title">{device.name}</h4>
      ...
      <Recommendations title="Можливо ви шукаєте" />
      <Recommendations title="Часто купують" />
      ...
      <div className={cn("device-carriage", { active: priceFixed })}>
        <div className="container">
          <div className="device-carriage__inner">
            <h4 className="device-carriage__title title">{device.name}</h4>
...
export const getServerSideProps = wrapper.getServerSideProps(
  () =>
    async ({ params }) => {
      const response = await devicesApi.getOne(params?.id);
      console.log(response);
      return {
        props: {
          serverDevice: response,
        },
      };
    }
);
